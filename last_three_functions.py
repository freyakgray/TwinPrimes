    ...
    for i in hexasNum: # cycle through all hexa pairs
        ...
        for j in SqSextands[i-1]: # cycle through critical area
            ...
            for k in i: # check modulos of all hexas for a given sextand
                if (j % hexas[k] ==  hexasSextands[k] or  j % hexas[k] == hexas[k] - hexasSextands[k]):
                    valid = False 
...
def generateCombos(hexasNum):
    ...
    while(adv):
        ...
        print(format.__format__(index) + ":")
        for i in range(len(combo)):
            combo[i] = index % hexas[i]
            ...
            if(i % 2 == 0):
                ...
                if(combo[i] == ((i + 1) / 2) or combo[i] == hexas[i] - ((i + 1) / 2)):
                    valid = False
